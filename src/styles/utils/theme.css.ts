...
const darkBlueBanner = createTheme(theme, {
  background:
    "light-dark(oklch(32.25% 0.0573 233.51),oklch(72.55% 0.0573 233.51))",
  backgroundHover:
    "light-dark(oklch(72.55% 0.0573 233.51),oklch(32.25% 0.0573 233.51))",
  primary: "white",
  secondary: "null",
...
const blueLightBg = createTheme(theme, {
  background:
    "light-dark(oklch(97.44% 0.0134 240.95),oklch(0.69 0.0345 247.76))",
...
for (const size of sizesContainerGridCol) {
  globalStyle(
    `${containerGridCol({ size: "default" })} > ${containerGridCol({ size })}`,
    {
      maxInlineSize: maxInlineSizeFn(size),
    },
  );
