...
import { component$, createContextId, type Signal, useContextProvider, useSignal, useStyles$ } from '@builder.io/qwik'
...
export default component$(() => {
  ...
  return (
    <nav class='bg-yellow-200'>
      ...
      >
        {menu.map((item, index) =>
          item.type === 'link' ? <Navlink key={index} {...item} /> : <Submenu key={index} idx={index} {...item} />,
        )}
      </ul>
    </nav>
  )
