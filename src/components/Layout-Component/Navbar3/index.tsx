...
export default component$(() => {
  ...
  return (
    <nav id='nav-bar' class='bg-sky-500/50 relative gap-3 max-md:grid flex flex-row items-center'>
      <div class='flex items-center justify-between p-4'>
        <h1 class='text-3xl md:pr-9 font-bold text-blue-800 hover:text-sky-400'>DentiCourb</h1>
        <button
      ...
      >
        {menu.map((item, index) =>
          item.type === 'link' ? (
            ...
            <Submenu key={index} changeTab={changePosition} idx={index} isMobile={true} {...item} />
          ),
        )}
      </ul>

      <ul class={['max-md:hidden md:flex md:justify-start gap-1 md:items-center transition-all duration-300']}>
        {menu.map((item, index) =>
          item.type === 'link' ? (
            <Navlink key={index} {...item} />
          ) : (
            <Submenu key={index} changeTab={changePosition} idx={index} isMobile={false} {...item} />
          ),
        )}
      </ul>
    </nav>
